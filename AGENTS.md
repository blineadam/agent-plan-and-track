...
- `node .github/scripts/check-digest-preview.js`: verify `rules/core-rules.md` still satisfies CI's digest-packing rules (the three priority bullets present, in order, inside the inline-preview budget, and the whole file under the inline-persistence ceiling) before pushing an edit to it (free).
...
The brevity-trims-elaboration and deliverable-purity bullets in both files are adapted from [alexgreensh/attention-span](https://github.com/alexgreensh/attention-span), whose output styles carry the same two ideas. Only the ideas are carried over, not the text: that project is AGPL-3.0 and this repo is MIT. Its scanning format, arrow-marked paragraphs and skim-the-bold-only emphasis, was deliberately left behind, since it conflicts with the writing-voice rule's ban on bold-lead bullets for short answers. `docs/out-of-scope.md` records that decline and three others from the same review.
...
`.ai-style-rules.md` (maintained by the `inherit-legacy-style` skill) captures this repo's own implicit conventions, and `copilot-review-instructions` regenerates `.github/instructions/*.instructions.md` (path-scoped Copilot PR-review directives) from it, along with the `# Code reviews` section of `.github/copilot-instructions.md`, which points Copilot's reviewer at those files and names what CI already blocks. Among the path-scoped files, one carrying the `<!-- Generated by copilot-review-instructions; ... -->` marker is fully skill-owned and safe to regenerate; one without it needs provenance confirmed with the user before overwriting, since absence of the marker doesn't necessarily mean hand-authored. Ownership works differently in `.github/copilot-instructions.md`, where the skill owns a section rather than the whole file: the marker sits under the `# Code reviews` heading, and heading-to-next-H1 bounds what may be rewritten. An unmarked section there is not assumed hand-authored either, but it is never adopted silently; its provenance is confirmed first. Everything outside the section is hand-authored guidance that also steers Copilot's cloud agent, which is why the nightly workflow below cannot write this file at all: its allowlist covers only the wholly-owned artifacts.

A nightly workflow (`.github/workflows/nightly-style-refresh.yml`) re-runs this loop unattended against `main` and merges the result through a Copilot-reviewed PR, so `.ai-style-rules.md` and the generated instruction files can change without a human commit. Its diff allowlist deliberately excludes `.github/copilot-instructions.md`. Every file it does admit carries `excludeAgent: "cloud-agent"`, so nothing an unattended session writes can reach Copilot's cloud agent; that file carries no such exclusion, and no diff check on it would help, since the risk is what gets written inside the owned section rather than outside it. The section is therefore refreshed only on a human-run invocation of the skill, and its list of path-scoped files can lag a nightly run that adds or drops a bucket.
