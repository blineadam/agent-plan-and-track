...
- Never name `tasks/todo.md`, `tasks/lessons.md`, or a batch number in a commit message, PR title/description, code comment, or release note: those files are gitignored and local-only, so the reference means nothing to anyone else. Describe the actual change instead.
