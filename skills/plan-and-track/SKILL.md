...
7. For an executor-tagged batch, don't check its boxes yet when the report comes back: first review just that batch's diff (the uncommitted working-tree changes, or the batch's commit range if commits already landed). On Claude Code, the bundled `code-review` skill carries `disable-model-invocation`, so a session cannot call it via the Skill tool: it runs only when the user types `/code-review`, so use it when they have and take the direct-review path below when they have not. On Codex, run the native review command scoped to the batch: `codex review --commit <sha>` for one commit, `codex review --base <branch>` for the branch diff, or `codex review --uncommitted` only when the batch owns the entire working-tree diff (concurrent batches share the tree). In an interactive Codex session, `/review` is the working-tree review path. Anywhere no batch-scoped native review can run, review the diff directly in the main session instead: reread it fresh for correctness bugs first, then reuse/simplification. On a high-risk diff (concurrency, security-sensitive surfaces, hard-to-reverse operations), that direct read does not count toward the two independent reviewers [[efficient-frontier]] requires: dispatch both. Either way, the review works from the diff and the plan's requirements, not the implementer's report or reasoning, and starts from the assumption the change is wrong, looking for how it fails; a pass that only confirms the implementer's summary isn't a review. Fix or explicitly triage/defer every finding, with a one-line reason appended directly to the relevant Plan item(s), not the `## Review` section (that heading's presence is step 1's proof that closeout already ran, so writing there early risks a premature compression), before checking any boxes; silence on a finding is not triage. Scope: executor-tagged batches only, not researcher (no diff), mechanic (fully pre-specified), or main (already under the main session's own judgment).
