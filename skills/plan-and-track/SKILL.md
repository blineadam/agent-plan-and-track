...
3. Enter plan mode if not already in it. Write a detailed spec upfront to reduce ambiguity:
   ...
   On Claude Code and Codex, spec drafting can be delegated to a `planner` subagent (Fable) and the finished checklist handed to an `executor` subagent (Sonnet) to implement; Copilot has no subagent concept, so plan inline there instead.
