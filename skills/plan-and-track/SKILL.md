...
7. For an executor-tagged batch, don't check its boxes yet when the report comes back: first dispatch an independent review of just that batch's diff (the uncommitted working-tree changes, or the batch's commit range if commits already landed). Use the built-in `/code-review` skill on Claude Code (self-invocable via the Skill tool) at its default effort; where that's unavailable (Codex, Copilot), delegate per [[efficient-frontier]] to a read-only reviewer whose handoff names the diff scope and asks for correctness bugs first, then reuse/simplification. Fix or explicitly triage/defer every finding, with a one-line reason recorded in the batch's Review section, before checking any boxes; silence on a finding is not triage. This gate applies to executor-tagged batches only, not researcher (returns no diff), mechanic (edits are fully pre-specified), or main (already under the main session's own judgment).
...
11. Add a `## Review` section to `tasks/todo.md` summarizing what changed, why, and how it was verified; record the batch's commit range or PR when one exists (e.g. `Commits: abc1234..def5678` or `PR #X`).
