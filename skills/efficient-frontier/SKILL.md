...
description: "Use before subagent selection/spawn or handback review: high-stakes security/architecture; plan drafting (task plan: plan-and-track); plan/spec/todo execution; research; bug repro; mechanical edits. Also use before committing to a high-stakes security or architecture decision, or reviewing a high-risk diff, even when no spawn is planned. Also use before taking a technical-judgment question of your own (which design, is this safe, better approach, second opinion) to the user: consult the fitting advisor first. Not for authorization, scope, or intent questions, which go straight to the user."
---
...
- Don't send two agents to edit the same files at the same time: give each parallel stream non-overlapping ownership of what it may write, since two writers to the same file clobber each other with no conflict signal. For the same reason, never run a command that mutates shared or out-of-scope state, such as a project-wide formatter, generator, or dependency update, from inside one parallel task while others are still in flight.
