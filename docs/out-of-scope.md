...
The decision covers both shapes the question raised. `copilot-review-instructions` does not generate a code-review skill alongside the two channels it already owns. And a `SKILL.md` whose body only points back at `.github/copilot-instructions.md` is the same thing the `REVIEW.md` entry above rejected: a third file with no content of its own, inviting drift against the two that have consumers.

What would reopen this: a review procedure rather than a rule, such as "run the digest-preview check and report its output" or "query this MCP server before commenting on hook changes". That is the task-specific work a skill exists for, and none exists here today.
...
- Replacing the installed `outputStyle: Concise` with one of the three styles. Concise is one of Claude Code's built-in styles and the [docs describe it](https://code.claude.com/docs/en/output-styles) as leading "with the result, skips preamble and narration, and keeps responses short by default", which is the same ground the upstream styles cover. `install.sh` already asserts it as a managed default. The same page notes that output styles reach the main conversation only, since a subagent runs its own system prompt, so a style is not a substitute for the rule files either way. Swapping it in would import the format conflict above and add a per-harness file the installers would then have to manage.
